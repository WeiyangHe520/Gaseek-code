...
def train_multitask_model(X_train, y_diag_train, y_mmr_train, y_type_train,
...
# ============================================================
cat("\n========================================\n")
cat("开始训练多任务模型（诊断 + MMR + 癌肿分型）...\n")
cat("========================================\n\n")

py_run_string("
model, history = train_multitask_model(
    X_train_final, y_diag_train_final, y_mmr_train_final, y_type_train_final,
    input_dim=23, epochs=100, lr=0.001, batch_size=128
)
")
...
# 保存结果
write.csv(results_df, "./multitask_diag_metrics.csv", row.names = FALSE)
cat("\n诊断任务结果已保存到: ./multitask_diag_metrics.csv\n")

# ============================================================
# 13. MMR任务评估（测试集，仅癌症样本）
# ============================================================
cat("\n========================================\n")
cat("多任务模型评估结果（MMR任务 - 测试集）\n")
cat("========================================\n")
...
if (length(cancer_idx_test) > 0) {
  ...
  if (sum(valid_mmr) > 0 && length(unique(mmr_true[valid_mmr])) > 1) {
    ...
    cat(sprintf("MMR任务: AUC = %.3f (95%% CI: %.3f-%.3f)\n", auc_mmr, ci_mmr[1], ci_mmr[3]))
    cat(sprintf("有效样本数: %d\n", sum(valid_mmr)))
  } else {
...
if (file.exists("./glmnet_evaluation_metrics_with_brier.csv")) {
  glmnet_results <- read.csv("./glmnet_evaluation_metrics_with_brier.csv")
  cat("\nglmnet测试集诊断AUC:", glmnet_results$AUC[2], "\n")
  cat("多任务测试集诊断AUC:", test_metrics$AUC, "\n")
  
  diff <- test_metrics$AUC - glmnet_results$AUC[2]
  cat(sprintf("差异: %.3f (%s)\n", abs(diff), ifelse(diff > 0, "多任务更高", "glmnet更高")))
} else {
  cat("未找到glmnet结果文件，跳过对比\n")
}
...
if (!is.null(train_loss_vec) && length(train_loss_vec) > 0) {
  loss_df <- data.frame(
    Epoch = 1:length(train_loss_vec),
    Train_Loss = train_loss_vec
  )
  
  p <- ggplot(loss_df, aes(x = Epoch, y = Train_Loss)) +
    geom_line(color = "blue", linewidth = 1) +
    labs(title = "训练过程 - 损失曲线", y = "Loss", x = "Epoch") +
    theme_minimal()
  
  ggsave("./multitask_loss_curve.png", p, width = 8, height = 6)
  cat("损失曲线已保存: ./multitask_loss_curve.png\n")
}
...
if (sum(valid_diag) > 0 && length(unique(test_labels$diag[valid_diag])) > 1) {
  png("./multitask_diag_roc.png", width = 6, height = 6, units = "in", res = 300)
  roc_diag <- roc(test_labels$diag[valid_diag], test_pred_diag[valid_diag])
  plot(roc_diag, main = paste0("诊断任务 ROC\nAUC = ", round(roc_diag$auc, 3)), 
       col = "blue", lwd = 2)
  dev.off()
  cat("诊断ROC曲线已保存: ./multitask_diag_roc.png\n")
}
