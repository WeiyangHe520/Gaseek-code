...
p_threshold <- ggplot(threshold_data_long %>% 
  ...
  geom_vline(xintercept = threshold_results$high_ppv$threshold, 
             linetype = "dashed", color = "#ff7f00", size = 1.2, alpha = 0.8) +
  ...
  geom_text_repel(
    ...
    nudge_y = c(-0.6, -0.5, -0.6)    # accuracy上移，PPV上移，NPV下移
  ) +
...
region_colors <- c("Confidence benign region" = "#377eb8", 
                   ...
                   "Confidence malignant region" = "#ff7f00")

label_colors <- c("benign" = "#377eb8", "malignant" = "#ff7f00")
...
p_distribution <- ggplot(pred_dist_df, aes(x = prediction, fill = true_label)) +
  ...
  labs(
    title = "Internal validation set (early-stage) 
    with confident cutoff",
    x = "Prediction value",
    y = "density"
  ) +
...
region_colors_overall <- c(
  ...
  "Positive area" = "#ff7f00"  # 红色
)

label_colors <- c("benign" = "#4daf4a", "malignant" = "#ff7f00")
