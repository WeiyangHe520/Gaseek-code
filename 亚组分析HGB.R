  ...
  for (var in clinical_features) {
    if (var %in% names(data)) {
      ...
      data[[paste0(var, "_group")]] <- factor(
        ifelse(data[[var]] > median_val, paste0(var, ">141g/L"), paste0(var, "≤141g/L")),
        levels = c(paste0(var, "≤141g/L"), paste0(var, ">141g/L"))
      )
    }
  }
...
# 如果HGB不在列中，请确保列名正确；此处假设存在HGB列
if ("HGB" %in% colnames(test_data_for_subgroup)) {
...
# 确保测试数据框存在
if (exists("test_data_for_subgroup") && "HGB" %in% colnames(test_data_for_subgroup)) {
  
  # 提取 HGB 列（可根据需要扩展为多个临床特征）
  var <- "HGB"
  data <- test_data_for_subgroup
  ...
  cat("注意：测试集中未找到'HGB'列，或 test_data_for_subgroup 不存在，跳过截断点生成。\n")
}