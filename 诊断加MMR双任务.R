...
def train_multitask_model(X_train, y_diag_train, y_mmr_train,
...
# 保存结果
write.csv(results_df, "./multitask_diag_metrics_diag_mmr.csv", row.names = FALSE)
cat("\n诊断任务结果已保存到: ./multitask_diag_metrics_diag_mmr.csv\n")
...
if (length(cancer_idx_test) > 0) {
  ...
  if (sum(valid_mmr) > 0 && length(unique(mmr_true[valid_mmr])) > 1) {
    ...
    write.csv(mmr_result, "./multitask_mmr_metrics_diag_mmr.csv", row.names = FALSE)
  } else {
...
if (!is.null(train_loss_vec) && length(train_loss_vec) > 0) {
  ...
  cat("损失曲线已保存: ./multitask_loss_curve_diag_mmr.png\n")
}
...
if (sum(valid_diag) > 0 && length(unique(test_labels$diag[valid_diag])) > 1) {
  ...
  plot(roc_diag, main = paste0("诊断任务 ROC (诊断+MMR)\nAUC = ", round(roc_diag$auc, 3)), 
       col = "blue", lwd = 2)
  dev.off()
  cat("诊断ROC曲线已保存: ./multitask_diag_roc_diag_mmr.png\n")
}
...
if (length(cancer_idx_test) > 0) {
  ...
  if (sum(valid_mmr) > 0 && length(unique(test_labels$mmr[cancer_idx_test][valid_mmr])) > 1) {
    ...
    plot(roc_mmr, main = paste0("MMR任务 ROC\nAUC = ", round(roc_mmr$auc, 3)), 
         col = "orange", lwd = 2)
    dev.off()
    cat("MMR ROC曲线已保存: ./multitask_mmr_roc_diag_mmr.png\n")
  }
}
