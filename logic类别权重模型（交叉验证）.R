...
# ============================================================
# 5折交叉验证函数（LogitBoost版本）
# ============================================================
perform_cv_logitboost <- function(X, y, weights, nIter, nfolds = 5) {
  set.seed(3456)
  ...
  for (i in 1:nfolds) {
    ...
    # 训练LogitBoost模型（注意：LogitBoost不支持weights参数）
    model_fold <- LogitBoost(
      x = X_train_fold,
      y = y_train_fold,
      nIter = nIter
    )
    ...
    # 预测验证集
    pred_prob <- predict(model_fold, X_val_fold, type = "raw")[, 2]
    ...
    # 使用验证集的权重（这里我们使用原始权重，但验证集可能需要调整）
    # 对于验证集，我们使用相等的权重或者基于类别的权重
    val_weights <- ifelse(y_val_fold == 1, weight_cancer, weight_control)
    
    for (th in thresholds) {
      pred_class <- ifelse(pred_prob > th, 1, 0)
      tp <- sum(val_weights[pred_class == 1 & y_val_fold == 1])
      fn <- sum(val_weights[pred_class == 0 & y_val_fold == 1])
      fp <- sum(val_weights[pred_class == 1 & y_val_fold == 0])
      tn <- sum(val_weights[pred_class == 0 & y_val_fold == 0])
    ...
    # 计算加权Brier评分
    brier_weighted <- sum(val_weights * (pred_prob - y_val_fold)^2) / sum(val_weights)
...
cv_output <- perform_cv_logitboost(
  X = train_matrix,
  y = train_num,
...
# 保存交叉验证结果
write.csv(cv_output$results, "./logitboost_weighted_cv_fold_results.csv", row.names = FALSE)
write.csv(cv_output$summary, "./logitboost_weighted_cv_summary.csv", row.names = FALSE)
...
# 保存模型和工作空间
# ============================================================
save(model_logitboost, cv_output, results_df, file = "./logitboost_weighted_model.rdata")
...
cat("LogitBoost模型摘要\n")
cat("========================================\n")
cat(sprintf("模型类型: LogitBoost\n"))
cat(sprintf("参数配置:\n"))
cat(sprintf("  - nIter (迭代次数): %d\n", best_nIter))
cat(sprintf("\n交叉验证 (5折) 平均性能:\n"))
for (i in 1:nrow(cv_output$summary)) {
  cat(sprintf("  - %s: %.3f (SD: %.3f)\n", 
              cv_output$summary$Metric[i],
