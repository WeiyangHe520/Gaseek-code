...
perform_subgroup_analysis_fixed <- function(data, predictions, true_labels, 
  ...
  binary_vars_originally <- c("gender", "Smoking_history", "drinking_history", 
  ...
  for (var in clinical_features) {
    if (var %in% names(data)) {
      ...
      # 对于gender，特殊处理（因为归一化后male和female的值是固定的）
      if (var == "gender" && is_originally_binary) {
        cat(sprintf("\n特征 '%s' 是二分类变量（已归一化）:\n", var))
        
        # 根据归一化前的值进行分组
        # 假设: female = -1.17217818, male = 0.852996108
        # 使用接近这些值的阈值来分组
        if (exists("gender_threshold")) {
        ...
        # 创建分组
        group_male <- paste0(var, "_male")
        group_female <- paste0(var, "_female")
        
        data[[paste0(var, "_group")]] <- factor(
          ifelse(data[[var]] > threshold, group_male, group_female),
          levels = c(group_female, group_male)
        )
        
        n_female <- sum(data[[var]] <= threshold, na.rm = TRUE)
        n_male <- sum(data[[var]] > threshold, na.rm = TRUE)
        
        cat(sprintf("  分组阈值: %.6f\n", threshold))
        cat(sprintf("  女性组 (%s): ≤ %.6f (n=%d)\n", group_female, threshold, n_female))
        cat(sprintf("  男性组 (%s): > %.6f (n=%d)\n", group_male, threshold, n_male))
        ...
        grouping_info <- rbind(grouping_info, data.frame(
          ...
          Group1_Name = group_female,
          Group1_Definition = sprintf("≤ %.6f (female)", threshold),
          Group1_N = n_female,
          Group2_Name = group_male,
          Group2_Definition = sprintf("> %.6f (male)", threshold),
          Group2_N = n_male,
          stringsAsFactors = FALSE
        ))
...
if (exists("y_test")) {
  ...
  gender_subgroup_results <- perform_subgroup_analysis_fixed(
  ...
  # 打印结果
  if (!is.null(gender_subgroup_results$performance_metrics)) {
    cat("\n性别亚组分析性能指标:\n")
    print(gender_subgroup_results$performance_metrics)
    
    cat("\n性别分组详细信息:\n")
    print(gender_subgroup_results$grouping_info)
  }
}