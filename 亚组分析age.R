  ...
  for (var in clinical_features) {
    if (var %in% names(data)) {
      ...
      data[[paste0(var, "_group")]] <- factor(
        ifelse(data[[var]] > median_val, paste0(var, ">47"), paste0(var, "≤47")),
        levels = c(paste0(var, "≤47"), paste0(var, ">47"))
      )
    }
  }
...
# 如果age不在列中，请确保列名正确；此处假设存在age列
if ("age" %in% colnames(test_data_for_subgroup)) {
...
# 确保测试数据框存在
if (exists("test_data_for_subgroup") && "age" %in% colnames(test_data_for_subgroup)) {
  
  # 提取 age 列（可根据需要扩展为多个临床特征）
  var <- "age"
  data <- test_data_for_subgroup
  ...
  cat("注意：测试集中未找到'age'列，或 test_data_for_subgroup 不存在，跳过截断点生成。\n")
}