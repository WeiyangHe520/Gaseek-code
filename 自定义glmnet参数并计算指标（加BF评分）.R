...
calculate_metrics_with_ci <- function(true_labels, predictions, probabilities, dataset_name) {
  ...
  if(length(unique(true_numeric)) < 2) {
    cat("警告：", dataset_name, "中只有一个类别，无法计算AUC和Brier评分\n")
    auc_value <- NA
    brier_score <- NA
  } else {
    # 计算ROC曲线
    ...
    # 计算Brier评分
    brier_score <- mean((probabilities - true_numeric)^2)
  }
...
results_df_detailed <- data.frame(
  ...
  Brier_Score = c(train_metrics$Brier, test_metrics$Brier),
...
print("\n模型性能汇总（包含AUC 95%置信区间和Brier评分）:")
print(results_df_detailed)

# 保存为CSV文件
csv_filename <- "./glmnet_evaluation_metrics_with_brier.csv"
...
# 训练集Brier评分可视化
plot(train_prob, train_true_numeric, 
     main = "训练集: 预测概率 vs 真实标签",
     xlab = "预测概率", ylab = "真实标签 (0/1)",
     pch = 19, col = rgb(0, 0, 1, 0.5))
abline(h = c(0, 1), lty = 2, col = "gray")
text(0.8, 0.2, sprintf("Brier = %.4f", train_metrics$Brier), cex = 1)

# 测试集Brier评分可视化
plot(test_prob, test_true_numeric, 
     main = "测试集: 预测概率 vs 真实标签",
     xlab = "预测概率", ylab = "真实标签 (0/1)",
     pch = 19, col = rgb(1, 0, 0, 0.5))
abline(h = c(0, 1), lty = 2, col = "gray")
text(0.8, 0.2, sprintf("Brier = %.4f", test_metrics$Brier), cex = 1)
...
# 生成简化的汇总报告
cat("\n=== 模型性能汇总报告 ===\n")
cat(sprintf("训练集: AUC = %.3f (95%% CI: %.3f-%.3f), Brier = %.4f, ACC = %.3f\n", 
            train_metrics$AUC, train_metrics$AUC_lower, train_metrics$AUC_upper, 
            train_metrics$Brier, train_metrics$ACC))
cat(sprintf("测试集: AUC = %.3f (95%% CI: %.3f-%.3f), Brier = %.4f, ACC = %.3f\n", 
            test_metrics$AUC, test_metrics$AUC_lower, test_metrics$AUC_upper, 
            test_metrics$Brier, test_metrics$ACC))
...
cat("• 接近0 = 模型校准良好\n")
cat("• 接近1 = 模型校准很差\n")
cat(sprintf("您的模型: 训练集Brier = %.4f, 测试集Brier = %.4f\n", 
            train_metrics$Brier, test_metrics$Brier))