...
# ============================================================
# 5. 定义 PyTorch 模型（Python部分）- 4任务版本
# ============================================================
py_run_string("
import torch
import torch.nn as nn
...
def train_multitask_model(X_train, y_diag_train, y_mmr_train, y_type_train, y_stage_train,
    ...
    unc_loss = UncertaintyWeightedLoss(num_tasks=4).to(device)
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_d, y_m, y_t, y_s in train_loader:
            ...
            total_loss = unc_loss(torch.stack([loss_d, loss_m, loss_t, loss_s]))
            optimizer.zero_grad()
            total_loss.backward()
...
cat("\n=== 诊断任务结果汇总 ===\n")
print(results_df)
write.csv(results_df, "./multitask_diag_metrics_4tasks.csv", row.names = FALSE)
...
if (!is.null(train_loss_vec) && length(train_loss_vec) > 0) {
  ...
  p <- ggplot(loss_df, aes(x = Epoch, y = Train_Loss)) +
    ...
    labs(title = "训练过程 - 损失曲线（4任务）", y = "Loss", x = "Epoch") +
    theme_minimal()
  
  ggsave("./multitask_loss_curve_4tasks.png", p, width = 8, height = 6)
  cat("损失曲线已保存: ./multitask_loss_curve_4tasks.png\n")
}
