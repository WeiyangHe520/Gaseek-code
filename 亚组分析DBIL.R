  ...
  for (var in clinical_features) {
    if (var %in% names(data)) {
      ...
      data[[paste0(var, "_group")]] <- factor(
        ifelse(data[[var]] > median_val, paste0(var, ">2.9μmol/L"), paste0(var, "≤2.9μmol/L")),
        levels = c(paste0(var, "≤2.9μmol/L"), paste0(var, ">2.9μmol/L"))
      )
    }
  }
        ...
        for (facet_group in facet_groups) {
          ...
          if (nrow(facet_data) >= 10 && length(unique(facet_data[[outcome_var]])) >= 2) {
            ...
            p_subgroup <- p_subgroup + 
              geom_text(data = data.frame(x = 1.5, y = y_position, label = sig_symbol, group_var = facet_group),
                        aes(x = x, y = y, label = label), inherit.aes = FALSE,
                        size = 5, fontface = "bold", color = ifelse(p_value < 0.05, "#e41a1c", "gray"))
          }
        }
...
# 如果DBIL不在列中，请确保列名正确；此处假设存在DBIL列
if ("DBIL" %in% colnames(test_data_for_subgroup)) {
...
# 确保测试数据框存在
if (exists("test_data_for_subgroup") && "DBIL" %in% colnames(test_data_for_subgroup)) {
  
  # 提取 DBIL 列（可根据需要扩展为多个临床特征）
  var <- "DBIL"
  data <- test_data_for_subgroup
  ...
  cat("注意：测试集中未找到'DBIL'列，或 test_data_for_subgroup 不存在，跳过截断点生成。\n")
}