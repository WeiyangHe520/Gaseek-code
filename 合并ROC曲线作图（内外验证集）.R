...
# 定义队列文件路径和标签
cohort_files <- list(
  "Internal validation cohort" = "glmnet_roc_curve_points（胃癌Train）.csv",
  "External validation cohort" = "glmnet_roc_curve_points（胃癌Test）.csv",
  "CITWH cohort" = "glmnet_roc_curve_points（结肠癌Train）.csv",
  "ZHWHU cohort" = "glmnet_roc_curve_points（结肠癌Test）.csv",
  "ZCH cohort" = "glmnet_roc_curve_points（ZCH）.csv"
)
...
# 为4个队列定义颜色方案
color_scheme <- c(
  "Internal validation cohort" = "#E41A1C",               # 红色
  "External validation cohort" = "#377EB8", # 蓝色
  "CITWH cohort" = "#4daf4a", # 亮黄色
  "ZHWHU cohort" = "#984ea3",
 "ZCH cohort" = "#ff7f00"# 浅绿色
)
