...
# 定义队列文件路径和标签
cohort_files <- list(
  "Training cohort (gastric cancer)" = "glmnet_roc_curve_points（胃癌Train）.csv",
  "Internal validation cohort (gastric cancer)" = "glmnet_roc_curve_points（胃癌Test）.csv",
  "Training cohort (colorectal cancer)" = "glmnet_roc_curve_points（结肠癌Train）.csv",
  "Internal validation cohort (colorectal cancer)" = "glmnet_roc_curve_points（结肠癌Test）.csv"
)
...
# 定义您想要的图例顺序
custom_order <- c(
  "Training cohort (gastric cancer)" ,
  "Internal validation cohort (gastric cancer)",
  "Training cohort (colorectal cancer)" ,
  "Internal validation cohort (colorectal cancer)"
)
...
# 为4个队列定义颜色方案
color_scheme <- c(
  "Training cohort (gastric cancer)" = "#E41A1C",               # 红色
  "Internal validation cohort (gastric cancer)" = "#377EB8", # 蓝色
  "Training cohort (colorectal cancer)" = "#FFD92F", # 亮黄色
  "Internal validation cohort (colorectal cancer)" = "#A6D854"  # 浅绿色
)
...
auc_summary_table <- auc_values %>%
  ...
  mutate(
    ...
    Cohort_Type = case_when(
      ...
      grepl("internal", Cohort, ignore.case = TRUE) ~ "SCH internal validation",
